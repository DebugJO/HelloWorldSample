...
# Visual Studio 2026 단축키

빠른 작업 및 리펙터링(수정) : `alt + Enter`
주석토글(수정) : `ctrl+alt+/`
검색 : 코드검색 : `ctrl + P`, `ctrl + T`(기존참고)
편집 문서 서식(수정) : `ctrl + alt + Enter`
코드 정리(주의) : `ctrl + K, E` : `.editorconfig`에 따라 전체 코드 정렬

### 라인 편집

|**기능**|**단축키**|**설명**|
|---|---|---|
|**줄 복사**|`Ctrl + D`|현재 줄(또는 블록)을 바로 아래에 복사 (매우 유용)|
|**줄 이동**|`Alt + ↑ / ↓`|현재 줄을 위/아래로 한 줄씩 이동 (코드 순서 바꿀 때 필수)|
|**줄 삭제**|`Ctrl + Shift + L`|현재 커서가 있는 줄 전체를 즉시 삭제|
|**한 줄 선택**|`Ctrl + 커서 줄 클릭`|(마우스 사용 시) 또는 커서를 줄 맨 앞에서 `Shift + ↓`|
|**줄 삽입**|`Ctrl + Enter`|커서 위치와 상관없이 바로 아래에 빈 줄 추가|
|**줄 삽입(위)**|`Ctrl + Shift + Enter`|커서 위치와 상관없이 바로 위에 빈 줄 추가|

### 탐색 및 선택

|**기능**|**단축키**|**설명**|
|---|---|---|
|**전체 이동**|**`Ctrl + T`**|파일, 클래스, 메서드 이름을 한 번에 검색|
|**단어 선택**|`Ctrl + W`|커서가 있는 단어 선택 (반복 시 선택 영역 확장)|
|**정의 이동**|`F12`|메서드나 변수가 선언된 곳으로 즉시 이동|
|**정의 피킹**|`Alt + F12`|화면 이동 없이 작은 창으로 선언부 확인 (매우 편리)|
|**참조 찾기**|`Shift + F12`|이 변수/메서드를 사용 중인 모든 곳을 리스트업|
|**이전 위치 이동**|`Ctrl + -`|방금 전에 코딩하던 위치로 되돌아가기 (뒤로가기)|
|텍스트 검색|`ctrl + shift + F` | |
...
|**기능**|**단축키**|**설명**|
|---|---|---|
|**이름 변경**|**`Ctrl + R, R`**|변수/메서드 이름을 프로젝트 전체에서 안전하게 변경|
|**빠른 수정**|**`Ctrl + .`** 또는 **`Alt + Enter`**|Roslynator 및 VS 제안(전구) 메뉴 열기|
|**멀티 커서**|`Ctrl + Alt + 클릭`|여러 곳에 커서를 찍어 동시에 타이핑|
|**세로 선택**|`Shift + Alt + 방향키`|박스 형태로 구역을 선택하여 일괄 편집|
|**주석 처리**|`Ctrl + K, C`|선택 영역 주석 처리 (Comment)|
|**주석 해제**|`Ctrl + K, U`|선택 영역 주석 해제 (Uncomment)|

### 화면 관리 및 기타

|**기능**|**단축키**|**설명**|
|---|---|---|
|**코드 정리**|**`Ctrl + K, E`**|**(설정 필수)** `.editorconfig`에 따라 전체 코드 정렬|
|**전체 화면**|`Shift + Alt + Enter`|에디터만 크게 보기 (집중 모드)|
|**파일 탭 닫기**|`Ctrl + F4`|현재 열려있는 파일 닫기|
|**솔루션 빌드**|`Ctrl + Shift + B`|프로젝트 전체 빌드|
...
|**생성 대상**|**키워드**|**실행 방식**|
|---|---|---|
|**생성자**|**`ctor`**|`ctor` 입력 후 **`Tab` 2번**|
|**속성(Property)**|**`prop`**|`prop` 입력 후 **`Tab` 2번** (이름 변경 가능)|
|**전체 속성**|`propfull`|필드와 함께 속성 생성 (get/set 포함)|
|**소멸자(Destructor)**|**`~`**|클래스명 앞에 `~` 치고 `Tab` (또는 수동 작성)|
|**예외 처리**|`try`|`try` 입력 후 **`Tab` 2번** (catch 블록 포함)|
|**for 문**|`for`|`for` 입력 후 **`Tab` 2번** (변수 자동 설정)|

### 바꾸기

심볼 이름 바꾸기 : `ctrl + R, R` : 의미 중심
단순 바꾸기 : `ctrl + H` : 글자 중심
