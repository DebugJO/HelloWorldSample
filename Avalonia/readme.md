...
### DispatcherPriority 주요 순위 정리

|**우선순위 이름**|**레벨**|**설명 및 용도**|
|---|---|---|
|**`Send`**|10|**가장 높음.** 다른 작업보다 먼저 즉시 실행해야 할 때 사용합니다.|
|**`Normal`**|9|**기본값.** 일반적인 작업(변수 변경, 로직 처리 등)이 실행되는 단계입니다.|
|**`DataBind`**|8|데이터 바인딩이 업데이트되는 시점입니다.|
|**`Render`**|7|**중요.** UI 요소들을 화면에 실제로 그리는(Layout/Render) 단계입니다.|
|**`Loaded`**|6|모든 비주얼 요소가 배치되고 컨트롤 로딩이 완료된 시점입니다.|
|**`Input`**|5|마우스 클릭, 키보드 입력 같은 사용자 이벤트를 처리하는 단계입니다.|
|**`Background`**|4|UI와 직접 관계없는 무거운 작업들을 뒤에서 몰래 처리할 때 좋습니다.|
